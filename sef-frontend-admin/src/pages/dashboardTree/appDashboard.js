...
import {FormDashboardImage} from "../forms/formDashboardImage";
...
export function AppDashboard(props) {
    ...
    const runStore = ( primaryKey ) => {
        ...
        const getOptions = ( loadOptions, isExport ) =>{
            ...
            console.log('treeNodeId', props );

            //  ------- PLACE GRID ID
            params += `tree_node_id=${ props.data.tree_node_id  }&`;
        ...
        let store = new CustomStore({
            ...
            update: (key, values) => {
                ...
                return securedDataManager.putAppDashboard(key,values).then((data) => {
            ...
            insert: (values) => {
                ...
                values['tree_node_id'] = props.data.tree_node_id;

                return securedDataManager.createDashboard(values).then((data) => {});

            },
            remove: (key) => {

                return securedDataManager.deleteDashboard(key).then((data) => {
            ...
            load(loadOptions) {
                ...
                return securedDataManager.gridAppTreeDashboard( loadOptions.isLoadingAll
    ...
    const cellRenderImage = (data) => {
        ...
        if(data.value){
            return <>
                    <img style={{ cursor: 'pointer'}} onClick={() => {IconClick(data)}}  width='100' src={'data:image/*;base64,' + data.value } />
                   </>;
        }else{
            return <span style={{ fontSize: '10px', fontWeight: 'bold', verticalAlign:'center' }} onClick={() => {IconClick(data)}}> <AddToPhotosIcon sx={{ cursor:'pointer', marginRight: '20px'}} /> </span>
        }
    ...
    const initDataGrid = () =>{
        ...
        return(
            <Grid container spacing={2} sx={{
                ...
                <Breadcrumbs sx={{ marginBottom: '30px', marginLeft: '30px', width: '35%', marginTop: '15px', cursor: 'pointer' }} aria-label="breadcrumb">
                    <Typography onClick={() => { props.back() }} sx={{ color: '#3399CC' }} color="text.primary">Groups</Typography>
                    <Typography  color="text.primary">Dashboards</Typography>
                </Breadcrumbs>
                ...
                >
                    ...
                    <div style={{ maxWidth: '100%' }}>
                        ...
                        >
                            ...
                            <Column
                                dataField={'dash_type_id'}
                                fixed={false}
                                caption={ 'Dashboard Type' }
                                width={150}
                                headerCellRender={renderTitleHeader}
                                alignment="left"
                                allowFiltering={true}
                            ...
                            >
                                <Lookup
                                    dataSource={[
                                        {
                                            value: 1,
                                            label: 'ESRI Dashboardr'
                                        },
                                        {
                                            value: 2,
                                            label: 'Arc GIS Insights'
                                        },
                                        {
                                            value: 3,
                                            label: 'DeckGl'
                            ...
                            <Column
                                dataField={'dash_esri_id'}
                                fixed={false}
                                caption={'Dashboard Esri Id'}
                                width={200}
                                headerCellRender={renderTitleHeader}
                                alignment="center"
                                allowFiltering={true}
                            ...
                            <Column  allowFiltering={false}
                                     ...
                                     caption={'Thumbnail'}
                                     fixed={false}
                                     dataField={'app_tree_node_content.thumbnail'}
                                     cellRender={cellRenderImage}
                            >
                                width={150}
                            </Column>

                            <Column width={100} fixed={false} type="buttons" >
    ...
    //  --------------------    UPLOAD COMPLETE
    //  --------------------
    const uploadedCompleted = () => {

        setOpen(false);

        gridExtreme.current.instance.refresh();
                ...
                >
                    ...
                    <DialogContent>
                        ...
                        <FormDashboardImage
                            ...
                            uploadedCompleted={uploadedCompleted}
                        />
